...
def test_child_with_computed():
    ...
    computed = source.pipe(operators.map(lambda x: x * 2))

    def create():
        return source.pipe(operators.with_latest_from(computed))

    results = scheduler.start(create)

    assert results.messages == [on_next(250, (10, 20)), on_next(350, (100, 200))]
